...
pub struct Builder {
    ...
    //处理服务器发给客户端的第一个包，因为没有包长
    first_packet_no_length_field : bool,
    is_server: bool,
}
...
impl LengthDelimitedCodec {
    ...
    fn decode_head(&mut self, src: &mut BytesMut) -> io::Result<Option<usize>> {
        ...
        let n = {
            ...
            // match endianess
            let mut n = if self.builder.length_field_is_big_endian {
                src.get_uint(field_len)
            } else {
                src.get_uint_le(field_len)
            };

            if !self.builder.is_server {
                //decode the head : length field
                let n0 = n.clone();
               // let n1 = n0.clone();
                let len = n0 as u16;
                let len_bytes: [u8; 2] = len.to_le_bytes();

                //println!("client: before encode: in length delimiter {} ", n0);

                let length = encode_head(&mut len_bytes.to_vec());
                let length_decode = [length[0],  length[1]];

                n = u64::from(u16::from_le_bytes(length_decode));
                //let n2 = n.clone();

                //let n3 : i16 = length_decode[0] as i16 | ( ( length_decode[1] as i16) << 8 ) as i16;
                //println!("client: after encode: in length delimiter {} {}", n2, n3);
            }
            else{
                //println!("server: in length delimiter {} ", n);
            }
...
impl Encoder for LengthDelimitedCodec {
    ...
    fn encode(&mut self, data: Bytes, dst: &mut BytesMut) -> Result<(), io::Error> {
        ...
        if self.builder.first_packet_no_length_field && !self.builder.is_server {
            self.builder.first_packet_no_length_field = false; //服务器发给客户端的第一个包没有长度信息，是固定一个int
           ...
           /* if self.builder.length_field_is_big_endian {
            ...
            if self.builder.is_server {
                //发给服务器
                //客户端给服务器是一个加密的被本encoder截取了包头（长度）的完整包，没有gatesdk添加额外的包头（长度）
                //本encoder接收的时候，把加密的包头（长度）去掉后扔给上层，
                //上层拿到后发给服务器时，添加加密包头，并且外面要包装一层包头（gatesdk解析用）
                dst.reserve( self.builder.length_field_len + n);
...
impl Builder {
    ...
    pub fn new() -> Builder {
        Builder {
            ...
            encoded : false,
            first_packet_no_length_field : true,
            is_server : false,
        }
    }
    ...
    pub fn first_packet_no_length_field(&mut self, b: bool) -> &mut Self {
        self.first_packet_no_length_field = b;
        self
    }

    pub fn encoded(&mut self, encode: bool) -> &mut Self {
        self.encoded = encode;
        self
    }

    pub fn is_server(&mut self, b: bool) -> &mut Self {
        self.is_server = b;
        self
    }
